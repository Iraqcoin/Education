(function($) {
	...
	$.avia_utilities.supports = ( function() {
			...
			// vendors   = ['Khtml', 'Ms','Moz','Webkit','O'];  exclude opera for the moment. stil to buggy
	...
	var listSwitcher = function() {
		...
		function switchToList() {
			...
			jQuery('.archive_switch').fadeOut(300, function() {
				...
				jQuery.cookie(cookie_name, 'list', {
					expires : 3,
					path : '/'
				});
			});
		...
		function switchToGrid() {
			...
			jQuery('.archive_switch').fadeOut(300, function() {
				...
				jQuery.cookie(cookie_name, 'grid', {
					expires : 3,
					path : '/'
				});
			});
	...
	var menu_landing = function() {
		...
		if (jQuery(".menu-scoll-landing").length) {
			...
			$('.tab-btns a[href*="#"]').on('click', function(event) {
				...
				var t = $(this);
				$('html, body').animate({
					scrollTop : $(this.hash).offset().top - $('.menu-scoll-landing').height()
				}, 500, function() {
